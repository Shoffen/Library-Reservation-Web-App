...
namespace LibraryReservationSystem.Controllers
{
    ...
    public class ReservationsController : ControllerBase
    {
        ...
        {
            double dailyRate = isAudiobook ? 2.0 : 3.0; // Audiobook rate is 2€, physical is 3€
            double subtotal = dailyRate * days;
            double totalCost = subtotal + serviceFee;

            if (quickPickup)
            {
                totalCost += pickupCost; // Add quick pickup cost
            }

            // Apply discounts
            if (days >= 3 && days < 10)
            {
                totalCost -= subtotal * 0.1; // 10% discount
            }
            else if (days >= 10)
            {
                totalCost -= subtotal * 0.2; // 20% discount
            }
